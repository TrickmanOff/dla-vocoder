...
def main(config):
    ...
    mel_spec_gen = MelSpectrogram(**config.get("mel_spectrogram_generator", {}))
    ...
    generator_loss_module = config.init_obj(config["generator"]["loss"], module_loss).to(device)
    discriminator_loss_module = config.init_obj(config["discriminator"]["loss"], module_loss).to(device)
